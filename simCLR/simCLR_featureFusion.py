...
from dataset_simCLR import ASOCT_Dataset, get_csv, split_csv_to_clients
...
def fine_tune():
    ...
    valid_dataset = ASOCT_Dataset(data_dir, img_size=config.image_size, train=False, transform=transform)
...
def feature_fusion():
    ...
    for client in range(total_step):
        ...
        model.load_state_dict(torch.load('simCLR_model_small_15.pt'))
        ...
        valid_dataset = ASOCT_Dataset(config.data_dir, img_size=config.image_size, train=False, transform=transform, return_dir=True)
        ...
        df = pd.DataFrame.from_dict(data_dict, orient='index', columns=['data_dir'] + ['feature_{}'.format(i) for i in range(128)])
        df.to_csv('simCLR_feature_fusion.csv', index=False)
